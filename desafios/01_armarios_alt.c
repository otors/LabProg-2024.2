...
typedef unsigned char TP; // Mude `char` para outro tipo caso queira armazenar mais dados 
#define SZ 8*sizeof(TP)

void mostrar_tabela(TP controle, unsigned char tempo); // Protótipo da função mostra_tabela

int main(int argc, char const *argv[])
{
    TP armarios = 0; // Inicialização da variavel de controle com armários desocupados
    char escolha;
    do {
        ...
        TP masc = 1; // Inicialização da mascara antes de cada operação
        if (escolha == 1) {
            ...
            char liberar;
            do {
                printf("Digite o armário que deseja liberar (0 a %ld): ", SZ-1);
                scanf("%hhd", &liberar);
            } while (liberar < 0 || liberar > (SZ-1));
...
*/
void mostrar_tabela(TP controle, unsigned char tempo) {
